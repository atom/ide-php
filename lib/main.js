    ...
    return new Promise((resolve, reject) => {
      ...
      childProcess.on('error', err => {
        ...
        atom.notifications.addError('IDE-PHP could not launch your PHP runtime.', {
          ...
          description: `${description}<p>If you have PHP 7 installed please Set PHP Path correctly. If you do not please Download PHP 7 and install it.</p><code>${err.message}</code>`
        ...
        if (exitCode === 0 && version != null) {
          ...
          if (Number(version.split('.')[0]) >= 7) {
            ...
            atom.notifications.addError(`IDE-PHP requires PHP 7 or later but found ${version}`, {
              ...
              description: 'If you have PHP 7 installed please Set PHP Path correctly. If you do not please Download PHP 7 and install it.'
            })
            reject()
          }
        } else {
