  ...
  checkPHPVersion (command) {
    return new Promise((resolve, reject) => {
      ...
      childProcess.on('error', err => {
        let description = 'If you have PHP 7 installed please set your PHP path. If you do not please download and install it.'
        if (this.getPHPPath() != null) {
          if (err.code == 'ENOENT') {
            description = `No PHP interpreter found at <b>${command}</b>. Please correct your PHP path.`
          } else {
            description = `Could not spawn the PHP interpreter configured at <b>${command}</b><code>${err.message}</code>`
          }
        }
        atom.notifications.addError('IDE-PHP could not launch your PHP runtime.', {
          dismissable: true,
          buttons: [
            { text: 'Download PHP 7', onDidClick: () => this.downloadPHP() },
      ...
      childProcess.stdout.on('data', chunk => version = chunk.toString())
      childProcess.on('exit', exitCode => {
        if (exitCode === 0 && version != null) {
          this.logger.debug(`found PHP version ${version} using "${command}"`)
          if (Number(version.split('.')[0]) >= 7) {
            resolve()
          } else {
            atom.notifications.addError(`IDE-PHP requires PHP 7 or later but found ${version}`, {
              dismissable: true,
              buttons: [
                { text: 'Download PHP 7', onDidClick: () => this.downloadPHP() },
                { text: 'Set PHP Path', onDidClick: () => this.openPackageSettings() },
              ],
              description: 'If you have PHP 7 installed please set your PHP path. If you do not please download and install it.'
            })
            reject()
          }
        } else {
